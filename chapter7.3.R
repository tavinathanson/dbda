...
newPrior = function(theta) {
  return((cos(4 * pi * theta) + 1)^2/1.5)
}

plot(newPrior)

# C. add newPrior to below:

# output histogram does look trimodal

# D. use non-empty data now

# data = 0.1, 1
# output histogram is now stronger around theta=0.5 but
# still some peaks at 0 and 1
# this is because bernoilli likelihood
# ...
# INCOMPLETE
# I was confused because I thought c(0.1, 1) vs. c(0,1,1)
# with the latter, the bern likelihood function
# is going to be skewed towards 2/3 but more focused on the middle
# so this output is a compromise between a 
# weakish prior and limited data

# E. D but with SD proposal=0.02
# posterior is wrong! weird peak at 0.1
# MORE:
# from solution, hard to get from one peak to another
# when SD is so low

# F. 
# new trajectory start = new local peak totally different
# from the peak in E, shows that there are issues
